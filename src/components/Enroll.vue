<template>
  <div class="IntelligentName">
      ...
      <div class="intelligent-form">
        ...
        <mt-field label="联系电话" placeholder="APP下单预留的电话" type="text" v-model="set.phone"></mt-field>
        ...
        <mt-popup v-model="set.areaVisible" class="area-class" position="bottom">
          <div class="picker-toolbar picker-toolbar-area" @click='dismiss()'><span class="mint-datetime-action mint-datetime-cancel">取消</span><span class="mint-datetime-action mint-datetime-confirm">确定</span></div>
      ...
      <div class="intelligent-form">
        <a class="mint-cell mint-field" style="min-height:.6rem;">
          <div class="mint-cell-wrapper">
            ...
            <div class="mint-cell-value">
              <input class="creat-text" type="text" placeholder="一个汉字" style="width:.75rem;height:24px;line-height:24px;font:inherit" v-model="set.singleName" id="singleInput">
        ...
        <mt-field label="忌讳字" placeholder="不超过5个中文汉字" type="text" v-model="set.limitName"></mt-field>
      </div>

      <mt-tabbar>
        <div class="set-meal-wraper">
          <div class="btn" @click="enrollSubmit">确认提交</div>
        </div>
      </mt-tabbar>
...
　　}
  ...
  methods: {
    ...
    prepay () {
      ...
      API.enroll(enrollParams).then(function (res) {
        console.log('======>',res);
        that.loading = false;
        if(res.code === 0) {
          setTimeout(() => {
            Indicator.close();
            Toast('提交成功，起名结果会尽快发给您，请耐心等待！');
          }, 2000);
        } else{
